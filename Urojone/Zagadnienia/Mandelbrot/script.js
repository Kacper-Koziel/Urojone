...
function checkIsInSet()
{
    ...
    if (match[2]) {
        ...
        imag = (imaginaryPart == "+i" || imaginaryPart == "i" ) ? 1 : imaginaryPart == "-i" ? -1 : parseFloat(imaginaryPart.replace('i', '')); 
    }

    if(!checkLimits(real, -100, 100) || !checkLimits(imag, -100, 100))
    {
    ...
    for (let i = 0; i < iterations; i++) {
        ...
        resultReal = realSqr + real;
        resultImg = imgSqr + imag;
...
document.getElementById("point").addEventListener("keydown", function (event) {
